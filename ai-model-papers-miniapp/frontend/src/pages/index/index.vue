...
import { getCapabilityList } from '@/utils/modelHelpers';
...
async function loadData(): Promise<void> {
  try {
    ...
    if (modelsRes.success) featuredModels.value = modelsRes.data.slice(0, 4);
    ...
    <view class="quick-actions">
      <view class="action-card action-card--primary" @click="goToModels">
        ...
        <view class="action-content">
          ...
          <text class="action-desc">12+ 主流模型</text>
        </view>
          ...
          >
            <view class="model-header">
              ...
              <view class="model-caps">
                <text 
                  v-for="cap in getCapabilityList(model.capabilities).slice(0, 2)" 
                  :key="cap.name"
                  class="model-cap"
                >{{ cap.icon }}</text>
              </view>
            ...
            <view class="model-footer">
              <view class="model-stat">
                <text class="model-stat-value">{{ model.quality.mmlu }}%</text>
                <text class="model-stat-label">MMLU</text>
              </view>
              <view class="model-price">
                <text class="model-price-value">${{ model.pricing.inputPrice }}</text>
                <text class="model-price-label">/1M</text>
              </view>
            </view>
          </view>
        </view>
